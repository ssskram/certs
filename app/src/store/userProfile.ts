...
export const actionCreators = {
  isUserAdmin: (user): AppThunkAction<any> => async dispatch => {
    const response = await fetch(
      "https://365proxy.azurewebsites.us/pghcerts/isAdmin?user=" + user.email,
      {
        method: "get",
        headers: new Headers({
  ...
  loadUserProfile: (user): AppThunkAction<any> => async dispatch => {
    const response = await fetch(
      "https://365proxy.azurewebsites.us/pghcerts/userProfile?user=" +
        user.email,
      {
        method: "get",
        headers: new Headers({
  ...
  updateUserProfile: (profile): AppThunkAction<any> => dispatch => {
    ...
    fetch(
      "https://365proxy.azurewebsites.us/pghcerts/userProfile?id=" + profile.id,
      {
        method: "PUT",
        body: JSON.stringify(forSP),
        headers: new Headers({
